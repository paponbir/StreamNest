...
export default function VideoCard({ video, onRemove, userRating }: VideoCardProps) {
  ...
  return (
    <Link href={`/watch/${video.id}?type=${mediaType}`}>
      <div className="relative min-w-[160px] md:min-w-[200px] h-[240px] md:h-[300px] cursor-pointer transition-all duration-500 hover:scale-[1.15] hover:z-50 bg-gray-900 rounded-lg overflow-hidden shadow-lg hover:shadow-[0_0_20px_rgba(229,9,20,0.5)] group">
        {onRemove && (
          <button 
            onClick={(e) => {
              e.preventDefault();
            ...
            className="absolute top-2 right-2 z-20 bg-black/60 hover:bg-red-600 rounded-full p-1 text-gray-300 hover:text-white transition-colors"
